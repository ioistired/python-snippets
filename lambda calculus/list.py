...
false = K = lambda x: lambda y: x
I = lambda x: x
true = K(I)

pair = lambda a: lambda b: lambda f: f(a)(b)
fst = lambda p: p(false)
snd = lambda p: p(true)

nil = pair(true)(true)
is_nil = fst
cons = lambda x: lambda l: pair(false)(pair(x)(l))
...
@_coconut_tco
def py_boolean(x):
    return _coconut_tail_call(x(False), True)
...
if_ = lambda p: lambda then: lambda else_: p(else_)(then)

fix = lambda f: lambda x: f(f)(x)
map = lambda fixed: lambda f: lambda xs: if_(is_nil(xs))(nil)(fixed(fixed)(f)(xs))
